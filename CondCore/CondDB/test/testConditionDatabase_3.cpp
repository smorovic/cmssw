...
int run( const std::string& connectionString ){
  try{
	...
	cond::time::lumiTime(100, 15), cond::time::lumiTime(100, 25), cond::time::lumiTime(100, 35), 
	cond::time::lumiTime(200, 15), cond::time::lumiTime(200, 25), cond::time::lumiTime(200, 35),
	cond::time::lumiTime(300, 15), cond::time::lumiTime(300, 25), cond::time::lumiTime(300, 35),
	cond::time::lumiTime(400, 11), cond::time::lumiTime(400, 12), cond::time::lumiTime(400, 13) };
    ...
    PayloadProxy<MyTestData> ppn;
    ppn.setUp(session);
    ppn.loadTag( "MyNewIOV" );
    for( auto t: inputTimes ){
      auto theIov = ppn.setIntervalFor( t );
      std::cout <<"NewPP: target="<<t<<" since="<<theIov.first<<" till:"<<theIov.second<<std::endl;
      ppn.invalidateCache();
    }

    std::cout <<"#NewPP: nqueries:"<<ppn.iov().numberOfQueries()<<std::endl;
