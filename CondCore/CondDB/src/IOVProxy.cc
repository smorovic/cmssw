...
namespace cond {
  ...
  namespace persistency {
    ...
    IOVArray::Iterator::Iterator( const Iterator& rhs ):
    ...
    const cond::Tag_t& IOVArray::tagInfo() const {
      return m_tagInfo;
    }

    IOVArray::Iterator IOVArray::begin() const {
      return Iterator( m_array->begin(),this );
    }
      
    IOVArray::Iterator IOVArray::end() const {
      return Iterator( m_array->end(),this );
    }
      
    IOVArray::Iterator IOVArray::find(cond::Time_t time) const {
      return Iterator( search( time, *m_array ), this );
    }

    size_t IOVArray::size() const {
      return m_array->size();
    }

      // returns true if at least one IOV is in the sequence.
    bool IOVArray::isEmpty() const {
    ...
    void IOVProxy::load( const std::string& tagName, 
      ...
      std::string dummy;
      if(!m_session->iovSchema().tagTable().select( tagName, m_data->tagInfo.timeType, m_data->tagInfo.payloadType, m_data->tagInfo.synchronizationType,
						    m_data->tagInfo.endOfValidity, dummy, m_data->tagInfo.lastValidatedTime ) ){
    ...
    void IOVProxy::loadGroups(){
      ...
      //checkTransaction( "IOVProxyNew::load" );
      m_session->iovSchema().iovTable().getGroups( m_data->tagInfo.name, m_data->snapshotTime, cond::time::sinceGroupSize( m_data->tagInfo.timeType ), m_data->sinceGroups );
    ...
    IOVArray IOVProxy::selectAll( const boost::posix_time::ptime& snapshottime ){
      if(!m_data.get()) throwException( "No tag has been loaded.","IOVProxy::selectAll"); 
      checkTransaction( "IOVProxy::load" );
      IOVArray ret;
      ret.m_tagInfo = m_data->tagInfo;
      m_session->iovSchema().iovTable().select( m_data->tagInfo.name, cond::time::MIN_VAL, cond::time::MAX_VAL, snapshottime, *ret.m_array );
      return ret;
    }
    ...
    IOVArray IOVProxy::selectRange( const cond::Time_t& begin, 
				       ...
				       const boost::posix_time::ptime& snapshotTime ){
      if(!m_data.get()) throwException( "No tag has been loaded.","IOVProxy::selectRange");
    ...
    cond::Tag_t IOVProxy::tagInfo() const {
      return m_data.get() ? m_data->tagInfo : cond::Tag_t();
    }
    ...
    std::tuple<std::string, boost::posix_time::ptime, boost::posix_time::ptime > IOVProxy::getMetadata() const {
      ...
      if(!m_session->iovSchema().tagTable().getMetadata( m_data->tagInfo.name, std::get<0>( ret ), std::get<1>( ret ), std::get<2>( ret ) ) ){
	throwException( "Metadata for tag \""+m_data->tagInfo.name+"\" have not been found in the database.","IOVProxy::getMetadata()");
      }
      return ret;
    }
    ...
    void IOVProxy::fetchSequence( cond::Time_t lowerGroup, cond::Time_t higherGroup ){
      m_data->iovSequence.clear();
      m_session->iovSchema().iovTable().select( m_data->tagInfo.name, lowerGroup, higherGroup, m_data->snapshotTime, m_data->iovSequence );
    ...
    cond::Iov_t IOVProxy::getInterval( cond::Time_t time ){
      return getInterval( time, cond::time::MAX_VAL );
    }
    
    cond::Iov_t IOVProxy::getInterval( cond::Time_t time, cond::Time_t defaultIovSize ){
      if(!m_data.get()) throwException( "No tag has been loaded.","IOVProxy::getInterval");
      checkTransaction( "IOVProxy::getInterval" );
      if(!m_data->cacheInitialized) loadGroups();
	...
	if( iGLow == m_data->sinceGroups.end() ){
	  ...
	  throwException( "0 In tag "+m_data->tagInfo.name+" can't find a valid interval for the specified time "+std::to_string(time),"IOVProxy::getInterval");
	}
	auto iGHigh = iGLow;
	...
	throwException( "1 In tag "+m_data->tagInfo.name+" can't find a valid interval for the specified time "+std::to_string(time),"IOVProxy::getInterval");
      }
      ...
      if( tillVal < retVal.till ) retVal.till = tillVal;
      //
    ...
    cond::Iov_t IOVProxy::getLast(){
      ...
      cond::Iov_t ret;
      bool ok = m_session->iovSchema().iovTable().getLastIov( m_data->tagInfo.name, m_data->snapshotTime, ret.since, ret.payloadId );

      if(ok) {
        if( m_data->tagInfo.endOfValidity < cond::time::MAX_VAL ){
	  if( ret.since >= m_data->tagInfo.endOfValidity ) {
	    ret.clear();
	  } else {
	    ret.till = m_data->tagInfo.endOfValidity;
	  }
	} else {
