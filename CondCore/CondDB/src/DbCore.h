...
namespace cond {
  ...
  template<typename... Columns> class ConditionBuffer {
  private:
  ...
  template <typename... Types> class BulkDeleter {
  public:
    static constexpr size_t cacheSize = 1000;
    ...
    void addStaticCondition( const std::string& condition ){
      m_buffer.addStaticCondition( condition );
    }

    template <typename P> void erase( const P& params ){
      m_buffer.set( params );
      if( !m_coralDeleter.get() ) m_coralDeleter.reset(  m_schema.tableHandle( m_tableName ).dataEditor().bulkDeleteRows( 
							 m_buffer.getClause(),m_buffer.get(), cacheSize ) );
      m_coralDeleter->processNextIteration(); 
    }

    void flush(){
      if( m_coralDeleter.get() ) m_coralDeleter->flush();
    }
  private:
    // fixme
    coral::ISchema& m_schema;
