...
void RunInfoHandler::getNewObjects() {
  ...
  edm::LogInfo( "RunInfoHandler" ) << "[" << "RunInfoHandler::" << __func__ << "]:" << m_name << ": "
                                   ...
                                   << ", hash " << tagInfo().lastInterval.payloadId << std::endl;
  ...
  if( !tagInfo().isEmpty()  && tagInfo().lastInterval.since >= m_since ) {
  ...
  if( !tagInfo().isEmpty()  && (tagInfo().lastInterval.since + 1) < m_since ) {
  ...
  if( !tagInfo().isEmpty() && m_since != 1 ) {
    appendToTransfer( (RunInfo*)(r->Fake_RunInfo()), 1 );
    //m_to_transfer.push_back( std::make_pair( (RunInfo*)(r->Fake_RunInfo()), 1 ) );
    ss << "fake run number: " << 1 << ", ";
  }
  if ( n_empty_run != 0 ) {
    appendToTransfer( (RunInfo*)(r->Fake_RunInfo()), tagInfo().lastInterval.since + 1 );
  ...
  *r = rn.readData( m_runinfo_schema, m_dcsenv_schema, (int)m_since );
  appendToTransfer( (RunInfo*)r, m_since  );
