...
namespace evf {
  ...
  void DTHFakeReader::fillRawData(edm::Event& e, FEDRawDataCollection*& data) {
    ...
    for (auto sourceId : sourceIdList_) {
      ...
      for (auto eventId : eventIdList_)
        size += randFedSizes[sourceId][eventId] + h_size_ + t_size_ + sizeof(DTHFragmentTrailer);
      feddata.resize(size);
      ...
      for (auto eventId : eventIdList_) {
        ...
        fragmentsSize += fillSLRFED(fedaddr, sourceId, eventId, orbitId, randFedSizes[sourceId][eventId], runningChecksum);
      }
  ...
  uint32_t DTHFakeReader::fillSLRFED(unsigned char* buf, const uint32_t sourceId, edm::EventNumber_t eventId, const uint32_t orbitId, uint32_t size, uint32_t &crc32c) {
    // Generate size...
    const unsigned h_size_ = sizeof(SLinkRocketHeader_v3);
    const unsigned t_size_ = sizeof(SLinkRocketTrailer_v3);

    uint32_t totsize = size + h_size_ + t_size_ + sizeof(DTHFragmentTrailer);
    ...
    new ((void*)(buf + h_size_ + size)) SLinkRocketTrailer_v3(crc, fragsize >> evf::word_num_bytes_shift, bxid, orbitId, crc, status);
    ...
    new (dthTrailerAddr) DTHFragmentTrailer(fragsize >> evf::word_num_bytes_shift , 0, crc, eventId);
    return totsize;
  }
  ...
  uint32_t DTHFakeReader::fillFED(unsigned char* buf, const int sourceId, edm::EventNumber_t eventId, uint32_t size, uint32_t &crc32c) {
    ...
    new (dthTrailerAddr) DTHFragmentTrailer((h_size + t_size + size) >> evf::word_num_bytes_shift, 0, crc, eventId);
    return totsize;
  }
